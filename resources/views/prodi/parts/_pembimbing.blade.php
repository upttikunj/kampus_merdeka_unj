<div class="peserta mb-2">
    <label class="col-form-label">Dosen Pembimbing</label><br/>
    <button class="btn btn-outline-primary" id="addDosPem" data-toggle="modal" data-target="#add-pembimbing-modal"><i class="fa fa-plus"></i> Dosen Pembimbing Kegiatan </button>
    <table class="table table-sm table-warning table-bordered" style="width: 100%">
      <thead>
        <tr>
          ...
          <th class="align-middle text-center bg-red bg-gradient text-white">Pembimbing ke</th>
      ...
      <tbody>
        ...
        ?>
            <tr>
                ...
                <td>{{ $dsn->pembimbing_ke }}</td>
                <td>{{ $namaKategori }}</td>
                <td>
                  <form action="{{ route('prodi.deleteDosenPembimbing', ['id' => $dsn->id]) }}" method="POST">
                    @csrf
                    @method('delete')
                    <button type="submit" data-toggle="tooltip" data-placement="top" title="Hapus Data" class="btn btn-sm btn-outline-danger" onclick="return confirm('Apakah Anda yakin akan menghapus Dosen Pembimbing ini ?')"><i class="fas fa-trash"></i> </button>
                  </form>
                </td>
            </tr>
        @endforeach
  ...
  @include("prodi.parts.addPembimbing")